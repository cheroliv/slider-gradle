...
`generateDeckContext` prépare un fichier de contexte pédagogique validé par l'auteur,
...
if (generateDeckContext ?) then (oui)
...
opt tâche RAG (generateDeckContext / generateDeck / collectRagIndex)
...
[source,bash]
----
./gradlew generateSlides
----

`asciidoctor`::
Lance la conversion Asciidoctor standard (dépend de `generateSlides`).
...
participant Gradle
participant deploySlides
participant "Build local"
participant Git

Développeur -> Gradle : ./gradlew deploySlides
Gradle -> deploySlides
deploySlides -> "Build local" : collecte les slides compilés
deploySlides -> Git : clone dépôt distant
deploySlides -> Git : copie les slides
deploySlides -> Git : commit
deploySlides -> Git : push (force)
@enduml
----
...
participant "Gradle\n(generateDeckContext)" as Propose
...
== Étape 1 : generateDeckContext ==

Développeur -> Propose : ./gradlew generateDeckContext\n  -Psubject="..." -Planguage=fr\n  -Pai.provider=gemini --no-daemon
Propose -> PGS : start()
activate PGS
...
./gradlew generateSlides serveSlides
----
...
[source,bash]
----
./gradlew cleanSlidesBuild generateSlides
----

.Publier vers le dépôt distant
[source,bash]
----
./gradlew generateSlides deploySlides
----
...
N'activez pas le Configuration Cache Gradle avec ce plugin — la tâche `generateSlides`
