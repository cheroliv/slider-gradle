...
=== `deploySlides` — configurable branch
...
- `collectRagIndex`, `generateDeckContext`, `generateDeck` (typed RAG tasks)
...
RT -> RM : retrieve(query) or reindex()
...
RT -> Files : write *-deck-context.yml\nor *_<lang>-deck.adoc
...
Properties available for `generateDeckContext`:
...
* `generateCapsule` — video capsule generation from slides.
