...
`generateDeckContext` prepares a pedagogical context file reviewed by the author,
...
if (generateDeckContext ?) then (yes)
...
opt RAG task (generateDeckContext / generateDeck / collectRagIndex)
...
`asciidoctor`::
Runs standard Asciidoctor conversion (depends on `generateSlides`).
...
Developer -> Gradle : ./gradlew deploySlides
Gradle -> deploySlides
deploySlides -> "Local build" : collect compiled slides
deploySlides -> Git : clone remote repository
deploySlides -> Git : copy slides
...
== Step 1: generateDeckContext ==

Developer -> Propose : ./gradlew generateDeckContext\n  -Psubject="..." -Planguage=en\n  -Pai.provider=gemini --no-daemon
Propose -> PGS : start()
activate PGS
...
Do not enable the Gradle Configuration Cache with this plugin — the `generateSlides` task
